...
class AdminRemovePlayer(BaseModel):
    user_id: int
...
def get_tournaments(db: Session = Depends(get_db)):
    ...
    for t in tournaments:
        ...
        res.append({"id": t.id, "name": t.name, "format_type": t.format_type, "rules": t.rules, "max_teams": t.max_teams, "teams_count": teams_count})
    return res

@app.post("/api/tournaments")
def create_tournament(data: TournamentCreate, db: Session = Depends(get_db)):
    t = models.Tournament(name=data.name, format_type=data.format_type, rules=data.rules, max_teams=data.max_teams, matchdays=data.matchdays)
    db.add(t)
    db.commit()
...
def generate_calendar(data: CalendarGenerate, db: Session = Depends(get_db)):
    ...
    regs = db.query(models.TournamentRegistration).filter(models.TournamentRegistration.tournament_id == t.id).all()
