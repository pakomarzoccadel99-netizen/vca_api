...
class ClubCreate(BaseModel): name: str; formation: str; logo_url: str = ""; owner_id: int
...
def login(data: UserAuth, db: Session = Depends(get_db)):
    ...
    if not user: raise HTTPException(status_code=400, detail="Credenziali errate.")
    hashed_input = hash_password(data.password)
    if user.password != hashed_input:
        if user.password == data.password: user.password = hashed_input; db.commit()
        else: raise HTTPException(status_code=400, detail="Credenziali errate.")
...
def set_role(data: RoleUpdate, db: Session = Depends(get_db)):
    ...
    user.role = data.new_role; db.commit()
...
def force_remove_player(data: AdminRemovePlayer, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == data.user_id).first()
    if user: user.club_id = None; user.role = "user" if user.role == "captain" else user.role; db.commit()
    return {"message": f"{user.gamertag} svincolato!"}

@app.get("/api/admin/users")
@app.get("/api/users/list")
def list_users(db: Session = Depends(get_db)):
    return [{"id": u.id, "gamertag": u.gamertag, "email": u.email, "role": u.role, "club_name": db.query(models.Club).filter(models.Club.id == u.club_id).first().name if u.club_id else None} for u in db.query(models.User).all()]
...
def create_club(data: ClubCreate, db: Session = Depends(get_db)):
    ...
    new_club = models.Club(name=data.name, formation=data.formation, logo_url=data.logo_url, owner_id=data.owner_id)
    db.add(new_club); db.commit(); db.refresh(new_club)
    owner = db.query(models.User).filter(models.User.id == data.owner_id).first()
    if owner: owner.club_id = new_club.id; owner.role = "captain"; db.commit()
...
def get_club(club_id: int, db: Session = Depends(get_db)):
    ...
    return {"name": club.name, "formation": club.formation, "logo_url": club.logo_url, "players": [{"user_id": p.id, "gamertag": p.gamertag, "role": p.role, "goals": p.goals, "assists": p.assists, "matches_played": p.matches_played} for p in players]}

@app.post("/api/club/add-player")
def add_player(data: PlayerAction, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == data.user_id).first(); user.club_id = data.club_id; db.commit(); return {"message": "Aggiunto"}

@app.post("/api/club/remove-player")
def remove_player(data: PlayerAction, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == data.user_id).first(); user.club_id = None; db.commit(); return {"message": "Rimosso"}
...
def create_tournament(data: TournamentCreate, db: Session = Depends(get_db)):
    db.add(models.Tournament(name=data.name, format_type=data.format_type, rules=data.rules, max_teams=data.max_teams, matchdays=data.matchdays, swiss_rounds=data.swiss_rounds, playoff_teams=data.playoff_teams, status="open"))
    db.commit()
...
def register_tournament(data: TournamentRegister, db: Session = Depends(get_db)):
    ...
    if db.query(models.TournamentRegistration).filter(models.TournamentRegistration.tournament_id == data.tournament_id, models.TournamentRegistration.club_id == data.club_id).first(): raise HTTPException(status_code=400, detail="Sei già iscritto o in coda.")
    is_waitlisted = db.query(models.TournamentRegistration).filter(models.TournamentRegistration.tournament_id == data.tournament_id, models.TournamentRegistration.is_waitlisted == False).count() >= t.max_teams
    db.add(models.TournamentRegistration(tournament_id=data.tournament_id, club_id=data.club_id, is_waitlisted=is_waitlisted))
    db.commit()
    return {"message": "Iscritti in LISTA D'ATTESA." if is_waitlisted else "Club iscritto ufficialmente!"}
...
def toggle_tournament(t_id: int, db: Session = Depends(get_db)):
    ...
    t.status = "closed" if t.status == "open" else "open"; db.commit()
...
def calculate_standings(t_id: int, db: Session):
    ...
    for r in regs:
        ...
        if club: stats[r.club_id] = {"club_id": club.id, "name": club.name, "logo_url": club.logo_url, "points": 0, "played": 0, "won": 0, "drawn": 0, "lost": 0, "gf": 0, "ga": 0, "gd": 0}
    ...
    for m in matches:
        if m.home_team_id in stats and m.away_team_id in stats:
            ...
            else: stats[m.home_team_id]["points"] += 1; stats[m.away_team_id]["points"] += 1; stats[m.home_team_id]["drawn"] += 1; stats[m.away_team_id]["drawn"] += 1
...
def generate_next_phase(data: CalendarGenerate, db: Session = Depends(get_db)):
    ...
    db.add_all([models.Match(tournament_id=t.id, home_team_id=h, away_team_id=a, matchday=100, play_date=play_date_str, phase=next_phase_name) for h, a in pairings])
    db.commit()
...
def get_pending_matches(club_id: int, db: Session = Depends(get_db)):
    res = []
    for m in db.query(models.Match).filter((models.Match.home_team_id == club_id) | (models.Match.away_team_id == club_id), models.Match.is_played == False).all():
...
def simulate_matches(t_id: int, db: Session = Depends(get_db)):
    for m in db.query(models.Match).filter(models.Match.tournament_id == t_id, models.Match.is_played == False).all():
...
def draft_signup(data: DraftSignup, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == data.user_id).first(); user.draft_role = data.role; db.commit()
    return {"message": f"Candidatura come {data.role} registrata!"}

@app.get("/api/draft/players")
def get_draft_players(db: Session = Depends(get_db)):
    return [{"id": p.id, "gamertag": p.gamertag, "role": p.draft_role} for p in db.query(models.User).filter(models.User.draft_role != None, models.User.club_id == None).all()]

@app.post("/api/draft/assign")
def draft_assign(data: PlayerAction, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == data.player_id).first(); user.club_id = data.club_id; user.draft_role = None; db.commit()
...
def get_top_players(db: Session = Depends(get_db)):
    ...
    return {"top_scorers": [format_p(p) for p in top_scorers if p.goals > 0], "top_assists": [format_p(p) for p in top_assists if p.assists > 0]}

@app.get("/api/tournaments/{t_id}/matches")
def get_tournament_matches(t_id: int, db: Session = Depends(get_db)):
    res = []
    for m in db.query(models.Match).filter(models.Match.tournament_id == t_id).order_by(models.Match.phase, models.Match.matchday).all():
        home = db.query(models.Club).filter(models.Club.id == m.home_team_id).first()
        away = db.query(models.Club).filter(models.Club.id == m.away_team_id).first()
        res.append({
            "id": m.id, "phase": m.phase, "matchday": m.matchday, "play_date": m.play_date,
            "home_team": home.name if home else "TBD", "home_logo": home.logo_url if home else "",
            "away_team": away.name if away else "TBD", "away_logo": away.logo_url if away else "",
